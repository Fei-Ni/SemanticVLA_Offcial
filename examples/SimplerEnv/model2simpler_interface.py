...
class SemanticVLAInference:
    def __init__(
        ...
        pass_step_to_server: bool = False,
        server_protocol: str = "legacy",
    ) -> None:
        ...
        self.unnorm_key = self.resolve_unnorm_key(norm_stats, unnorm_key)
        ...
        self.pass_step_to_server = bool(pass_step_to_server)
    ...
    def reset(self, task_description: str) -> None:
        ...
        self.sticky_gripper_action = 0.0
        self.previous_gripper_action = None
        self.latest_server_payload = None
    ...
    def step(
        ...
        progress_reset_episode: Optional[bool] = None,
        *args,
        **kwargs,
        ...
        if self.server_protocol == "policy_wrapper":
            vla_input = {
                "examples": [
                    {
                        "image": [image],
        ...
        elif self.server_protocol == "legacy":
            vla_input = {
                "batch_images": [[image]],
            ...
            if self.pass_step_to_server and step is not None:
                vla_input["step"] = int(step)
            if self.pass_step_to_server and progress_reset_episode is not None:
                vla_input["progress_reset_episode"] = bool(progress_reset_episode)
            if self.record_progress_variants or bool(kwargs.get("record_progress_variants", False)):
                vla_input["record_progress_variants"] = True
        else:
            raise ValueError(f"Unsupported server_protocol={self.server_protocol!r}")
        ...
        if self.server_protocol == "policy_wrapper":
            if "actions" not in response_data:
                raise RuntimeError(f"Malformed policy-wrapper response without actions: {response!r}")
            raw_actions = np.asarray(response_data["actions"][0], dtype=np.float32)
        elif "normalized_actions" in response_data:
        ...
        self.latest_server_payload = response_data
    ...
    def get_action_stats(unnorm_key: str, policy_ckpt_path) -> dict:
        ...
        # unnorm_key = baseframework._check_unnorm_key(norm_stats, unnorm_key)  # left commented; this is environment-specific
        return norm_stats[unnorm_key]["action"]

    @staticmethod
    def resolve_unnorm_key(norm_stats: Dict[str, dict], requested_key: Optional[str]) -> str:
        if requested_key in norm_stats:
            return requested_key
        if len(norm_stats) == 1:
            fallback_key = next(iter(norm_stats.keys()))
            print(
                f"*** requested unnorm_key {requested_key!r} not found; "
                f"falling back to sole available key {fallback_key!r} ***"
            )
            return fallback_key
        raise KeyError(
            f"unnorm_key {requested_key!r} not found; available keys: {list(norm_stats.keys())}"
        )
