...
DEFAULT_KEYFRAME_JSON = (
    "/home/n84416302/molmoact-modified/experiments/"
    "merged_fractal_ep000000_087211_jobs58584_58609_58610_58613.json"
)
DEFAULT_DATASET_PATH = "/home/n84416302/dataset/fractal/fractal20220817_data/0.1.0"
DEFAULT_OUTPUT_JSON_NAME = "complete_trajectories_fractal_dense_trace.json"

logger = logging.getLogger("run_fractal_dense_trace")
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path to the Fractal Stage1 sparse keyframe JSON",
    )
    parser.add_argument(
        "--dataset_path",
        type=str,
        default=DEFAULT_DATASET_PATH,
        help="Fractal RLDS dataset root",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
...
def run_pipeline(args: argparse.Namespace, minimal_outputs: bool = False) -> None:
    ...
    for episode_idx in tqdm(episode_indices, desc="Episodes"):
        ...
        if len(keyframes) < 2:
            logger.warning("Episode %s has too few keyframes; skipping", episode_idx)
            continue

        frames = load_episode_frames(episode_idx, args.dataset_path)
        fused: np.ndarray
        analysis: Dict
        candidates: List[BidirectionalTrack] = []
        ...
        if args.last4cotracker and not args.puremath:
        ...
        if not args.puremath and not minimal_outputs:
