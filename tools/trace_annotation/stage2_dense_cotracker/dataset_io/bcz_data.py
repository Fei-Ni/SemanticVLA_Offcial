...
_DATASET_INDEX_CACHE: Dict[Tuple[str, str], Tuple[List[str], Optional[List[int]], Optional[List[int]]]] = {}
...
def _build_dataset_index(
    ...
    tfrecord_files = sorted(
        [
            ...
            and (
                ...
                or f"-{split_name}_" in name
                or f"{split_name}.tfrecord" in name
                or (split_name == "train" and "fractal20220817_data" in name)
            )
        ]
    )
    if not tfrecord_files:
        # Fallback: take all tfrecord files (e.g. Fractal where filenames don't contain "train")
        tfrecord_files = sorted(
            [
                name
                for name in os.listdir(dataset_path)
    ...
    shard_lengths = load_shard_lengths_from_dataset_info(dataset_path, len(tfrecord_files), split_name)
    ...
    if shard_lengths is not None:
        ...
        logger.info("building index from dataset_info.shardLengths")
    else:
...
def _get_dataset_index(
    ...
    cache_key = (dataset_path, split_name)
    cached = _DATASET_INDEX_CACHE.get(cache_key)
    if cached is None:
        cached = _build_dataset_index(dataset_path, split_name)
        _DATASET_INDEX_CACHE[cache_key] = cached
    return cached
...
def _resolve_episode_location(
    ...
    tfrecord_files, _shard_lengths, shard_prefix = _get_dataset_index(dataset_path, split_name)
...
def load_episode_frames(
    ...
    split_episode_idx = episode_idx - episode_index_offset
    if split_episode_idx < 0:
        raise ValueError(
            f"Episode {episode_idx} is below split offset {episode_index_offset}"
        )

    tfrecord_name, file_idx, local_episode_idx = _resolve_episode_location(
        split_episode_idx,
        dataset_path,
        split_name,
    )
