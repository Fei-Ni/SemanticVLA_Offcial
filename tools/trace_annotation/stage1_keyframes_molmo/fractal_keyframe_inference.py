...
_NON_TRAIN_TOKENS = ("-val.", "-eval.", "-test.", "_val.", "_eval.", "_test.")


def _is_train_tfrecord(name: str) -> bool:
    """True if filename looks like a train-split tfrecord shard.

    BC-Z bundles `bc_z-val.tfrecord-*` alongside `bc_z-train.tfrecord-*` in the
    same directory; the dataset_info.json shardLengths covers only the train
    split, so mixing in val shards breaks the shard_prefix mapping. Exclude any
    file whose name carries a non-train split token.
    """
    if not (name.endswith(".tfrecord") or ".tfrecord-" in name):
        return False
    return not any(tok in name for tok in _NON_TRAIN_TOKENS)


def _list_train_tfrecord_files(dataset_path: str) -> List[str]:
    return sorted(name for name in os.listdir(dataset_path) if _is_train_tfrecord(name))


def get_total_episodes(dataset_path: str) -> int:
    """Return the total episode count for the dataset."""
    tfrecord_files = [
        os.path.join(dataset_path, name) for name in _list_train_tfrecord_files(dataset_path)
    ]
    if not tfrecord_files:
        raise ValueError(f"no tfrecord files found under {dataset_path}")
    
    total_episodes = 0
    logger.info(f"scanning {len(tfrecord_files)} tfrecord files for total episode count...")
...
def load_shard_lengths_from_dataset_info(dataset_path: str, expected_num_shards: int) -> Optional[List[int]]:
...
def build_target_tfrecord_index_set(
    ...
    sorted_targets = sorted(target_episode_set)
...
def iter_episode_data(
    ...
    tfrecord_files = _list_train_tfrecord_files(dataset_path)
    total_tfrecords = len(tfrecord_files)

    shard_lengths = load_shard_lengths_from_dataset_info(dataset_path, total_tfrecords)
    shard_prefix = None
    ...
    for file_idx in selected_file_indices:
        ...
        try:
            ...
            for episode in parsed_dataset:
                if shard_prefix is not None:
                    global_episode_idx = shard_prefix[file_idx] + local_episode_idx
                elif episodes_per_tfrecord > 0:
                    global_episode_idx = file_idx * episodes_per_tfrecord + local_episode_idx
                else:
                    global_episode_idx = fallback_episode_idx
...
def main():
    ...
    if args.total_nodes > 1 and args.end_episode is None:
        try:
            total_episodes = get_total_episodes(args.dataset)
            ...
            if total_episodes < args.total_nodes:
                ...
                if args.current_node >= total_episodes:
                    ...
                    args.start_episode = args.current_node
                    args.end_episode = args.current_node + 1
            else:
    ...
    results_file = os.path.join(
        args.output_dir,
        f"keyframe_results_{run_tag}_tf{args.start_tfrecord_idx:04d}_{tfrecord_end_tag}_ep{args.start_episode:06d}_{range_end_tag}.json",
    )
