#!/usr/bin/env python3
"""Repack the Bridge TraceX LeRobot component into LeRobot v3.0 layout.

This is an experimental release-path script.  It starts from our full-preserve
Bridge v2-style package, groups many episodes per parquet/video file, and
renames the trace representation from `observation.trace.xy` to scalar
...
def _write_readme(out_root: Path, info: dict[str, Any], repo_id: str) -> None:
    video_keys = sorted(key for key, spec in info["features"].items() if spec.get("dtype") == "video")
    text = f"""---
pretty_name: SemanticVLA Bridge TraceX v3
tags:
- robotics
- lerobot
- bridge
...
# SemanticVLA Bridge TraceX v3

Experimental LeRobot v3.0 repack of the Bridge TraceX component.  This package
preserves the Bridge TraceX episode order and raw observation/action fields from
the v2-style release package, while reducing the file count by grouping multiple
episodes per parquet/video file.
...
This is a side experiment to compare upload and hosting behavior against the
expanded v2-style Bridge TraceX package.  Use of this dataset is subject to the
original BridgeData V2 terms, plus the terms of the added SemanticVLA TraceX
annotations.
"""
...
def main() -> int:
    ...
    _write_readme(out_root, info_v3, args.repo_id)
