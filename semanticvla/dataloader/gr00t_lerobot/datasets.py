...
from semanticvla.dataloader.gr00t_lerobot.video import get_all_frames, get_frames_by_indices, get_frames_by_timestamps

from semanticvla.dataloader.gr00t_lerobot.embodiment_tags import EmbodimentTag
from semanticvla.dataloader.gr00t_lerobot.schema import (
    DatasetMetadata,
    DatasetStatisticalValues,
    LeRobotModalityMetadata,
    LeRobotStateActionMetadata,
)
from semanticvla.dataloader.gr00t_lerobot.transform import ComposedModalityTransform
from semanticvla.dataloader.gr00t_lerobot.transform.state_action import StateActionTransform
...
LE_ROBOT_SOURCE_STATS_FILENAME = "meta/stats.json"
...
class LeRobotSingleDataset(Dataset):
    ...
    def __init__(
        ...
        detected_version = detect_lerobot_version(self._dataset_path)
        ...
        # Build traj_id → length lookup.
        self._traj_length_map = {
            int(tid): int(tlen)
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        if modality_meta_path.exists():
            ...
            le_modality_meta = self._build_modality_meta_from_info()
        ...
        source_stats_path = self.dataset_path / LE_ROBOT_SOURCE_STATS_FILENAME
        try:
            ...
            if source_stats_path.exists():
                print(f"Falling back to LeRobot source statistics: {source_stats_path}")
    ...
    def _build_modality_meta_from_info(self) -> LeRobotModalityMetadata:
        features = self.lerobot_info_meta.get("features", {})
        if "observation.state" not in features or "action" not in features:
            raise FileNotFoundError(
                f"Missing {LE_ROBOT_MODALITY_FILENAME} and cannot infer state/action splits from meta/info.json"
            )

        state_dim = int(features["observation.state"]["shape"][0])
        action_dim = int(features["action"]["shape"][0])
        if state_dim < 14 or action_dim < 14:
            raise ValueError(f"Expected at least 14 state/action dims, got state={state_dim}, action={action_dim}")

        modality = {
            "state": {
                "left_joints": {"start": 0, "end": 6, "original_key": "observation.state", "dtype": "float32", "absolute": True},
                "right_joints": {"start": 7, "end": 13, "original_key": "observation.state", "dtype": "float32", "absolute": True},
                "left_gripper": {"start": 6, "end": 7, "original_key": "observation.state", "dtype": "float32", "absolute": True},
                "right_gripper": {"start": 13, "end": 14, "original_key": "observation.state", "dtype": "float32", "absolute": True},
            },
            "action": {
                "left_joints": {"start": 0, "end": 6, "original_key": "action", "dtype": "float32", "absolute": True},
                "right_joints": {"start": 7, "end": 13, "original_key": "action", "dtype": "float32", "absolute": True},
                "left_gripper": {"start": 6, "end": 7, "original_key": "action", "dtype": "float32", "absolute": True},
                "right_gripper": {"start": 13, "end": 14, "original_key": "action", "dtype": "float32", "absolute": True},
            },
            "video": {
                "cam_high": {"original_key": "observation.images.camera_h"},
                "cam_left_wrist": {"original_key": "observation.images.camera_l"},
                "cam_right_wrist": {"original_key": "observation.images.camera_r"},
            },
            "annotation": {
                "human.action.task_description": {"original_key": "task_index"},
            },
        }
        return LeRobotModalityMetadata.model_validate(modality)
    ...
    def _get_trajectories(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        if self._lerobot_version == "v3.0":
            ...
            if not episode_paths:
                raise FileNotFoundError(f"No v3 episode parquet files found under {self.dataset_path / 'meta/episodes'}")
            for file_path in episode_paths:
                episodes_data = pd.read_parquet(file_path)
                timestamp_cols = [
                    col for col in episodes_data.columns
                    if str(col).startswith("videos/") and str(col).endswith("/from_timestamp")
                ]
                for row_index, episode in episodes_data.iterrows():
                    ...
                    self.trajectory_ids_to_metadata[episode_index] = {
                        ...
                        "data/file_from_index": int(row_index),
    ...
    def _get_all_steps(self) -> list[tuple[int, int]]:
        ...
        # Preserve the legacy hard-coded cache path for v2 datasets, but keep
        # LeRobot v3 caches keyed by the real external contract so horizon/index
        # changes cannot silently reuse stale step lists.
    ...
    def _get_steps_config_key(self) -> str:
        ...
        action_horizon = max(
            (len(self.delta_indices[key]) for key in self.modality_keys.get("action", [])),
            default=0,
        )
        config_dict = {
            "delete_pause_frame": self.delete_pause_frame,
            ...
            "trajectory_index_mode": "episode_reset_v1" if self._lerobot_version == "v3.0" else "legacy",
        }
    ...
    def _get_lerobot_modality_meta(self) -> LeRobotModalityMetadata:
        ...
        if not modality_meta_path.exists():
            if self._lerobot_version == "v3.0" and self.data_cfg is not None and self.data_cfg.get("action_type") == "abs_qpos":
                return self._build_modality_meta_from_info()
            raise FileNotFoundError(f"Please provide a {LE_ROBOT_MODALITY_FILENAME} file in {self.dataset_path}")
        with open(modality_meta_path, "r") as f:
    ...
    def _get_data_path(self, trajectory_id: int) -> Path:
        if self._lerobot_version == "v3.0":
            episode_meta = self.trajectory_ids_to_metadata[int(trajectory_id)]
    ...
    def get_trajectory_data(self, trajectory_id: int) -> pd.DataFrame:
        ...
        parquet_path = self._get_data_path(trajectory_id)
        assert parquet_path.exists(), f"Parquet file not found at {parquet_path}"
        data = pd.read_parquet(parquet_path)
        if self._lerobot_version == "v3.0" and "episode_index" in data.columns:
            data = data.loc[data["episode_index"] == trajectory_id].copy().reset_index(drop=True)
        return data
    ...
    def get_video(
        ...
        codebase_version = self.lerobot_info_meta.get("codebase_version", "")
        if codebase_version == "v3.0-shadow-episode" and "frame_index" in self.curr_traj_data.columns:
            frame_indices = self.curr_traj_data["frame_index"].to_numpy()[step_indices].astype(np.int64)
            frame_backend = self.video_backend
            frame_backend_kwargs = self.video_backend_kwargs
            if frame_backend == "torchvision_av":
                # torchvision VideoReader timestamp seek is too coarse for short
                # episode-local clips; use exact frame indexing instead.
                frame_backend = "opencv"
                frame_backend_kwargs = {}
            return get_frames_by_indices(
                video_path.as_posix(),
                frame_indices,
                video_backend=frame_backend,
                video_backend_kwargs=frame_backend_kwargs,
            )
    ...
    def get_state_or_action(
        ...
        padding_strategy_cfg = str(
            _cfg_get(self.data_cfg, "state_action_padding_strategy", "zero")
        ).lower()
        if padding_strategy_cfg in {"yxz", "first_last_absolute", "absolute_first_last"}:
            padding_strategy = "first_last" if state_or_action_cfg.absolute else "zero"
        elif padding_strategy_cfg in {"zero", "first_last"}:
            padding_strategy = padding_strategy_cfg
        else:
            raise ValueError(f"Invalid state_action_padding_strategy: {padding_strategy_cfg}")
    ...
    def get_language(
        ...
        for i in range(len(step_indices)):
            task_indices.append(self.curr_traj_data[original_key].iloc[int(step_indices[i])].item())
    ...
    def save_dataset_statistics(self, save_path: Path | str, format: str = "json") -> None:
        ...
        if hasattr(self.metadata.statistics, 'action') and self.metadata.statistics.action:
            ...
            if filtered_action_stats:
                ...
                action_stats_mask_mode = str(
                    _cfg_get(self.data_cfg, "action_stats_mask_mode", "gripper_false")
                ).lower()
                if action_stats_mask_mode == "normalization":
                    action_normalization_modes = get_action_normalization_modes_for_used_keys(
                        self.transforms, filtered_action_stats.keys()
                    )
                elif action_stats_mask_mode == "gripper_false":
                    action_normalization_modes = None
                else:
                    raise ValueError(f"Invalid action_stats_mask_mode: {action_stats_mask_mode}")
...
def generate_action_mask_for_used_keys(
    ...
    By default this keeps the existing Isambard behavior: gripper-related
    keys are False. When action_normalization_modes is provided, only binary
    normalized dimensions are False, matching the yxz deployment contract.
    ...
    Args:
        ...
        action_normalization_modes: Optional clean action-key to normalization-mode mapping.
    ...
    for subkey in used_action_keys_ordered:
        if subkey in action_modalities:
            ...
            if action_normalization_modes is None:
                is_masked = "gripper" in subkey.lower()
            else:
                is_masked = action_normalization_modes.get(subkey) == "binary"
...
class LeRobotMixtureDataset(Dataset):
    ...
    def __init__(
        ...
        sequential_step_sampling: bool = False,
        action_stats_mask_mode: str = "gripper_false",
        ...
        self._sequential_step_sampling = _as_bool(sequential_step_sampling, default=False)
        self.action_stats_mask_mode = str(action_stats_mask_mode).lower()
        ...
        if self._sequential_step_sampling:
            for dataset in self.datasets:
                step_order = np.arange(len(dataset.all_steps))
                if self.mode == "train":
                    rng = np.random.default_rng(self.seed)
                    rng.shuffle(step_order)
                self._step_order.append(step_order)
    ...
    def sample_step(self, index: int) -> tuple[LeRobotSingleDataset, int, int]:
        ...
        if not self._sequential_step_sampling:
            ...
            if step_pos >= len(dataset.all_steps):
                step_order = np.arange(len(dataset.all_steps))
                if self.mode == "train":
                    order_seed = safe_hash((self.epoch, dataset_index, self.seed, step_pos))
                    order_rng = np.random.default_rng(order_seed)
                    order_rng.shuffle(step_order)
                self._step_order[dataset_index] = step_order
                step_pos = 0
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        """
        max_retries = 1 if self.strict_sample_errors else max(self.max_retries, 1)
        last_exception = None
        ...
        for attempt in range(max_retries):
            try:
                ...
                sample = dict(
                    ...
                    trajectory_id=int(trajectory_name),
                    step=int(step),
                )
    ...
    def save_dataset_statistics(self, save_path: Path | str, format: str = "json") -> None:
        ...
        if self.action_stats_mask_mode == "normalization":
            action_normalization_modes = {}
            for dataset in self.datasets:
        ...
        elif self.action_stats_mask_mode == "gripper_false":
            action_normalization_modes = None
        else:
            raise ValueError(f"Invalid action_stats_mask_mode: {self.action_stats_mask_mode}")
