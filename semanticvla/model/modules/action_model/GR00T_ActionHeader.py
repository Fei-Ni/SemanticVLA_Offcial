...
# Modification: [rm and add some connect adapter to match with semanticvla, e.g., "rm "].
...
from semanticvla.model.modules.action_model.flow_matching_head.action_encoder import (
    SinusoidalPositionalEncoding,
    swish,
)

from semanticvla.model.modules.action_model.flow_matching_head.cross_attention_dit import DiT
...
class FlowmatchingActionHeadConfig(PretrainedConfig):
    ...
    clamp_sample_time: bool = field(
        default=False, metadata={"help": "Clamp sampled Beta noise by noise_s before time conversion."}
    )
...
class FlowmatchingActionHead(nn.Module):
    ...
    def sample_time(self, batch_size, device, dtype):
        ...
        if bool(getattr(self.config, "clamp_sample_time", False)):
            sample = sample.clamp(max=self.config.noise_s)
    ...
    def predict_action(
        ...
        # Use a fixed RNG seed when do_sample=False so evaluation is repeatable.
