...
def _rewrite_base_vlm_path(global_cfg: dict) -> dict:
    framework_cfg = global_cfg.get("framework", {})
    qwenvl_cfg = framework_cfg.get("qwenvl", {})
    if not isinstance(qwenvl_cfg, dict):
        return global_cfg

    env_base_vlm = os.environ.get("BASE_VLM")
    if env_base_vlm:
        env_base_vlm_path = Path(env_base_vlm).expanduser()
        if not env_base_vlm_path.is_dir():
            raise FileNotFoundError(f"BASE_VLM directory does not exist: {env_base_vlm_path}")
        qwenvl_cfg["base_vlm"] = str(env_base_vlm_path.resolve())
        overwatch.info(f"Using BASE_VLM override: {qwenvl_cfg['base_vlm']}")
        return global_cfg

    base_vlm = qwenvl_cfg.get("base_vlm")
    if not isinstance(base_vlm, str) or not base_vlm:
        return global_cfg

    base_vlm_path = Path(base_vlm).expanduser()
    if base_vlm_path.is_absolute():
        return global_cfg

    repo_root = Path(__file__).resolve().parents[3]
    repo_relative_path = (repo_root / base_vlm_path).resolve()
    if repo_relative_path.is_dir():
        qwenvl_cfg["base_vlm"] = str(repo_relative_path)
        overwatch.info(f"Resolved relative base_vlm to local path: {qwenvl_cfg['base_vlm']}")
    return global_cfg
...
def read_model_config(pretrained_checkpoint):
    ...
    if os.path.isfile(pretrained_checkpoint):
        ...
        global_cfg = _rewrite_base_vlm_path(global_cfg)
