...
"""
SemanticVLA’s trainer is built directly on native PyTorch + Accelerate + DeepSpeed, keeping the loop explicit and easy to hack.
Conventions:
...
from semanticvla.training.trainer_utils.trainer_tools import TrainerUtils
from semanticvla.training.trainer_utils.trainer_tools import build_param_lr_groups
...
class VLATrainer(TrainerUtils):
    def __init__(self, cfg, model, vla_train_dataloader, optimizer, lr_scheduler, accelerator):
        ...
        self._warned_skip_multigpu_eval = False
        self.wandb_run = None
    ...
    def prepare_training(self):
        ...
        if freeze_modules:
            # Rebuild optimizer/scheduler after freezing so DeepSpeed only sees trainable params.
    ...
    def _log_metrics(self, metrics):
        ...
        if self.completed_steps % self.config.trainer.logging_frequency == 0:
            if dist.get_rank() == 0:
                ...
                # record to W&B when initialization succeeded
    ...
    def eval_action_model(self, step_metrics: dict = None) -> float:
        ...
        # In-train eval currently pulls an extra batch from the train iterator and only
        # executes predict_action on rank 0. That is safe enough for single-process debug,
        # but under multi-GPU DeepSpeed it can desynchronize ranks around eval boundaries.
        if self.accelerator.num_processes > 1:
            if self.accelerator.is_main_process and not self._warned_skip_multigpu_eval:
                logger.warning(
                    "Skipping in-train action eval for multi-process SemanticVLA jobs. "
                    "Use offline eval jobs instead."
                )
                self._warned_skip_multigpu_eval = True
            self.accelerator.wait_for_everyone()
            return step_metrics
    ...
    def _finalize_training(self):
        """training end processing"""
        # save final model
        skip_final_save = getattr(self.config.trainer, "skip_final_save", False)
        if isinstance(skip_final_save, str):
            skip_final_save = skip_final_save.lower() in {"true", "1", "yes", "on"}
